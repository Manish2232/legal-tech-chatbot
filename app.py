...
#             html, body {{ background-color: #070f1c !important; }}
#             .stApp {{ {background_css} background-color: #070f1c; }}
...
#         [data-testid="stAppViewContainer"], [data-testid="stMain"], [data-testid="stMainBlockContainer"] {{ background: transparent !important; }}
#         [data-testid="stHeader"] {{ background: transparent; }}
#         [data-testid="stBottom"], [data-testid="stBottomBlockContainer"] {{ background: #070f1c !important; }}
#         input, textarea, select {{ color-scheme: dark; }}
#         [data-testid="stTextArea"] textarea, [data-testid="stTextInput"] input {{
#             background: rgba(9, 19, 33, .9) !important; color: #f2f8ff !important;
#             border: 1px solid rgba(83, 230, 255, .2) !important; border-radius: 10px !important;
#         }}
#         [data-testid="stTextArea"] textarea:focus, [data-testid="stTextInput"] input:focus {{
#             border-color: rgba(83, 230, 255, .6) !important;
#             box-shadow: 0 0 0 3px rgba(83, 230, 255, .14) !important;
#         }}
#         [data-testid="stTextInput"] label p, [data-testid="stTextArea"] label p {{ color: #9fb8d1 !important; }}
#         [data-testid="stAlert"] {{ background: rgba(12, 19, 31, .92) !important; border: 1px solid rgba(83, 230, 255, .18) !important; }}
#         [data-testid="stAlert"] p {{ color: #f2f8ff !important; }}
...
#         /* ---------- Blanket override: no input/textarea can ever render light ---------- */
#         textarea, input {{
#             background-color: rgba(9, 19, 33, .9) !important;
#             color: #f2f8ff !important;
#             -webkit-appearance: none !important;
#             appearance: none !important;
#             caret-color: #53e6ff !important;
#         }}
#         textarea::placeholder, input::placeholder {{ color: #7f93ac !important; opacity: 1 !important; }}
#         /* Chrome/mobile autofill repaints background+text with its own colors unless forced via inset shadow */
#         textarea:-webkit-autofill, textarea:-webkit-autofill:hover, textarea:-webkit-autofill:focus,
#         input:-webkit-autofill, input:-webkit-autofill:hover, input:-webkit-autofill:focus {{
#             -webkit-text-fill-color: #f2f8ff !important;
#             -webkit-box-shadow: 0 0 0 1000px rgba(9, 19, 33, .95) inset !important;
#             box-shadow: 0 0 0 1000px rgba(9, 19, 33, .95) inset !important;
#             caret-color: #53e6ff !important;
#             transition: background-color 9999s ease-in-out 0s;
#         }}
#         /* Chrome's native "invalid field" red outline overriding our border */
#         textarea:invalid, input:invalid {{ box-shadow: none !important; outline: none !important; }}
#         [data-testid="stChatInput"]:has(textarea:invalid),
#         [data-testid="stChatInput"]:has(textarea:focus) {{ border-color: rgba(83, 230, 255, .6) !important; }}
#         [data-testid="stSidebar"] {{ background: linear-gradient(180deg, rgba(6, 16, 30, .97) 0%, rgba(4, 11, 21, .97) 100%); border-right: 1px solid rgba(83, 230, 255, .12); }}
#         .lexassist-brand-title {{ font-family: 'Inter', sans-serif; font-weight: 700; letter-spacing: .3px; background: linear-gradient(90deg, #ffffff 0%, #9beaff 45%, #53c8ff 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; filter: drop-shadow(0 0 6px rgba(83, 230, 255, .35)); margin: 0; line-height: 1.2; }}
#         .lexassist-brand-title.main {{ font-size: 1.9rem; }}
#         .lexassist-logo-img-sidebar {{ display: block; margin: 0 auto .3rem; max-width: 220px; width: 100%; height: auto; }}
...
Design notes
------------
This file intentionally injects ONE consolidated stylesheet (GLOBAL_CSS) right
after the page config, instead of scattering separate <style> blocks across
functions. That single stylesheet is what makes the dark theme independent of
the visitor's OS/browser light-or-dark preference: every color-bearing element
(inputs, textareas, buttons, alerts, tabs, popovers, the chat box) is pinned to
a fixed dark palette with `color-scheme: dark` set at the root, so browsers
never fall back to native light-mode form-control rendering.

Drop these two image files next to this script for branding:
  - lexassist.png  -> shown at the top of the login / create-account card
  - wallpaper.png  -> used as the full-page background image
Both are optional; the app still looks correct (just without imagery) if
...
def _b64_asset(filename: str) -> str | None:
    """Return base64 content of an image sitting next to this script, or None."""
...
LOGO_AUTH_B64 = _b64_asset("lexassist.png")
WALLPAPER_B64 = _b64_asset("wallpaper.png")

WALLPAPER_CSS = ""
if WALLPAPER_B64:
    WALLPAPER_CSS = f"""
        background-image:
            linear-gradient(rgba(4, 9, 18, 0.90), rgba(4, 9, 18, 0.96)),
            url('data:image/png;base64,{WALLPAPER_B64}');
        background-size: cover;
        background-position: center;
        background-attachment: fixed;
    """
...
# --------------------------------------------------------------------------- #
# Global stylesheet — the single source of styling truth for the whole app.
# Forces a dark theme that does NOT depend on the visitor's OS/browser setting.
# --------------------------------------------------------------------------- #
...
/* ============ Hard dark-mode lock, independent of OS/browser setting ============ */
:root, html, body {{
    color-scheme: dark;
    color-scheme: only dark;
}}
html, body {{ background-color: #070f1c !important; }}
* {{ scrollbar-color: rgba(83, 230, 255, .3) transparent; }}
...
[data-testid="stSidebar"] {{
    background: linear-gradient(180deg, rgba(6,16,30,.97) 0%, rgba(4,11,21,.97) 100%) !important;
    border-right: 1px solid rgba(83,230,255,.12);
}}
[data-testid="stSidebar"] * {{ color: #d7e8fa; }}
[data-testid="stSidebar"] .stCaption, [data-testid="stSidebar"] p {{ color: #8fa8c2 !important; }}
[data-testid="stSidebar"] h3 {{ color: #dceeff !important; font-size: .92rem; text-transform: uppercase; letter-spacing: .8px; font-weight: 600; opacity: .85; }}
[data-testid="stSidebar"] hr {{ border-color: rgba(83,230,255,.12); margin: 1.1rem 0; }}
[data-testid="stSidebar"] .stButton > button {{
    background: rgba(15,32,52,.65) !important; border: 1px solid rgba(83,230,255,.18) !important;
    color: #e6f4ff !important; border-radius: 10px; font-weight: 500; text-align: left; transition: all .15s ease;
}}
[data-testid="stSidebar"] .stButton > button:hover {{
    background: rgba(83,230,255,.12) !important; border-color: rgba(83,230,255,.5) !important;
    color: #ffffff !important; transform: translateX(1px);
}}
[data-testid="stSidebar"] .stButton > button:first-child {{
    background: linear-gradient(90deg, rgba(83,230,255,.18), rgba(185,255,240,.12)) !important;
    border-color: rgba(83,230,255,.55) !important; font-weight: 600;
}}
.lexassist-logo-img-sidebar {{ display: block; margin: 0 auto .3rem; max-width: 220px; width: 100%; height: auto; }}
.credit-box {{
    margin: .35rem 0 1.4rem; padding: 1rem .8rem; border: 1px solid #53e6ff; border-radius: 12px;
    background: rgba(5,25,43,.82); box-shadow: 0 0 7px #53e6ff, 0 0 18px rgba(83,230,255,.85), inset 0 0 12px rgba(83,230,255,.18);
    color: #edfcff !important; font-weight: 600; line-height: 1.55; text-align: center; text-shadow: 0 0 7px rgba(83,230,255,.9);
}}

/* ============ Brand title (main chat header) ============ */
.lexassist-brand-title {{
    font-family: 'Inter', sans-serif; font-weight: 700; letter-spacing: .3px;
    background: linear-gradient(90deg, #ffffff 0%, #9beaff 45%, #53c8ff 100%);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
    filter: drop-shadow(0 0 6px rgba(83,230,255,.35)); margin: 0; line-height: 1.2;
}}
.lexassist-brand-title.main {{ font-size: 1.9rem; }}
.stApp > header + div .stCaption {{ color: #90a9c4 !important; }}
...
.chat-row.user {{ justify-content: flex-end; }}
.chat-row.assistant {{ justify-content: flex-start; }}
.chat-text {{ font-size: 1.3rem; line-height: 1.85; overflow-wrap: anywhere; color: #f2f8ff; }}
.chat-row.user .chat-text {{ max-width: 68%; padding: .15rem .8rem; color: #b9fff0; text-align: right; }}
.chat-row.assistant .chat-text {{ width: 100%; max-width: 100%; padding: .15rem .8rem; color: #f2f8ff; text-align: left; }}

[data-testid="stHorizontalBlock"] {{ align-items: center !important; }}
[data-testid="stHorizontalBlock"] [data-testid="stColumn"] {{ display: flex !important; align-items: center !important; justify-content: center !important; }}
[data-testid="stHorizontalBlock"] [data-testid="stColumn"] [data-testid="stElementContainer"],
[data-testid="stHorizontalBlock"] [data-testid="stColumn"] [data-testid="element-container"] {{ margin: 0 !important; }}
[data-testid="stHorizontalBlock"] iframe {{ display: block !important; }}

/* ============ Popover (⋮ menu) ============ */
[data-testid="stPopover"] {{ display: flex !important; align-items: center !important; }}
[data-testid="stPopover"] > button, [data-testid="stPopover"] button {{
    min-width: 26px !important; min-height: 26px !important; width: 26px !important; height: 26px !important;
    padding: 0 !important; margin: 0 !important; background: transparent !important; border: none !important;
    border-radius: 50% !important; box-shadow: none !important; color: #8fa8c2 !important; font-size: 15px !important;
    line-height: 1 !important; transition: all .15s ease !important;
}}
[data-testid="stPopover"] > button:hover, [data-testid="stPopover"] button:hover {{
    background: rgba(92,184,255,.18) !important; color: #ffffff !important;
}}
[data-testid="stPopover"] > button svg, [data-testid="stPopover"] button svg {{ display: none !important; }}
[data-testid="stPopoverBody"] {{
    background: rgba(9,19,33,.98) !important; border: 1px solid rgba(83,230,255,.18) !important;
    border-radius: 12px !important; box-shadow: 0 8px 28px rgba(0,0,0,.45) !important;
}}
[data-testid="stPopoverBody"] * {{ color: #eafcff !important; }}

/* ============ Buttons (general) ============ */
.stDownloadButton > button, div[data-testid="stPopoverBody"] .stButton > button {{
    background: rgba(83,230,255,.1) !important; border: 1px solid rgba(83,230,255,.3) !important;
    color: #eafcff !important; border-radius: 8px; font-weight: 500; transition: all .15s ease;
}}
.stDownloadButton > button:hover, div[data-testid="stPopoverBody"] .stButton > button:hover {{
    background: rgba(83,230,255,.22) !important; border-color: rgba(83,230,255,.55) !important;
}}

/* ============ Chat input ============ */
[data-testid="stChatInput"] {{
    border: 1px solid rgba(83,230,255,.25) !important; background: rgba(9,19,33,.92) !important;
    border-radius: 14px !important; box-shadow: 0 4px 18px rgba(0,0,0,.3);
}}
...
/* ============ Blanket lock: no input/textarea/select can ever render light ============ */
textarea, input, select {{
    background-color: rgba(9,19,33,.92) !important;
    color: #f2f8ff !important;
    border-color: rgba(83,230,255,.18) !important;
    -webkit-appearance: none !important;
    appearance: none !important;
    caret-color: #53e6ff !important;
    color-scheme: dark;
...
textarea:invalid, input:invalid {{ box-shadow: none !important; outline: none !important; }}
/* Chrome/mobile autofill repaints its own background+text unless forced via inset shadow */
textarea:-webkit-autofill, textarea:-webkit-autofill:hover, textarea:-webkit-autofill:focus,
input:-webkit-autofill, input:-webkit-autofill:hover, input:-webkit-autofill:focus {{
    -webkit-text-fill-color: #f2f8ff !important;
    -webkit-box-shadow: 0 0 0 1000px rgba(9,19,33,.95) inset !important;
...
[data-testid="stTextInput"] label p, [data-testid="stTextArea"] label p {{ color: #9fb8d1 !important; font-weight: 500; }}

/* ============ Alerts (error / success / info) ============ */
[data-testid="stAlert"] {{ background: rgba(12,19,31,.92) !important; border: 1px solid rgba(83,230,255,.18) !important; border-radius: 10px; }}
[data-testid="stAlert"] p, [data-testid="stAlert"] span {{ color: #f2f8ff !important; }}
[data-testid="stAlert"] svg {{ fill: #53e6ff !important; }}

/* ============ Code blocks (verification codes) ============ */
.stCodeBlock, [data-testid="stCodeBlock"] {{ background: rgba(6,14,26,.9) !important; border: 1px solid rgba(83,230,255,.2) !important; }}
[data-testid="stCodeBlock"] code {{ color: #9beaff !important; }}

/* ============ Dividers / headings text ============ */
hr {{ border-color: rgba(83,230,255,.14) !important; }}
[data-testid="stMarkdownContainer"] p, [data-testid="stMarkdownContainer"] li,
[data-testid="stMarkdownContainer"] h1, [data-testid="stMarkdownContainer"] h2,
[data-testid="stMarkdownContainer"] h3 {{ color: #eafcff; }}
...
/* ============ Auth screen ============ */
[data-testid="block-container"] {{ padding-top: 2.4rem; }}
.auth-wrap {{ max-width: 920px; margin: 1.6rem auto 0; }}
.auth-shell {{
    display: grid; grid-template-columns: 1fr 1.15fr; align-items: stretch;
    background: rgba(10,17,29,.90); border: 1px solid rgba(83,230,255,.16); border-radius: 22px;
    box-shadow: 0 30px 70px rgba(0,0,0,.45), 0 0 0 1px rgba(83,230,255,.03);
    overflow: hidden; backdrop-filter: blur(10px);
}}
.auth-brand-panel {{
    position: relative; padding: 2.6rem 2.2rem;
    background: radial-gradient(120% 120% at 0% 0%, rgba(83,230,255,.16) 0%, rgba(9,17,29,0) 60%),
                linear-gradient(160deg, rgba(15,32,52,.9) 0%, rgba(7,15,27,.95) 100%);
    border-right: 1px solid rgba(83,230,255,.1);
    display: flex; flex-direction: column; justify-content: center; gap: 1rem;
}}
.auth-logo-img {{ max-width: 280px; width: 80%; height: auto; margin-bottom: .3rem; filter: drop-shadow(0 0 16px rgba(83,230,255,.35)); }}
.auth-brand-icon {{ font-size: 2.4rem; filter: drop-shadow(0 0 14px rgba(83,230,255,.55)); line-height: 1; }}
.auth-brand-name {{
...
.auth-brand-tagline {{ color: #a9c3dd; font-size: .97rem; line-height: 1.6; margin: 0; }}
.auth-badge {{
    display: inline-flex; align-items: center; gap: .4rem; width: fit-content; padding: .32rem .7rem; margin-top: .3rem;
    background: rgba(83,230,255,.1); border: 1px solid rgba(83,230,255,.3); border-radius: 999px;
    color: #bdeeff; font-size: .74rem; font-weight: 600; letter-spacing: .4px; text-transform: uppercase;
}}
.auth-feature-list {{ margin-top: 1rem; display: flex; flex-direction: column; gap: .5rem; }}
.auth-feature {{ display: flex; align-items: flex-start; gap: .55rem; color: #91acc7; font-size: .86rem; line-height: 1.5; }}
.auth-feature-dot {{ color: #53e6ff; margin-top: .1rem; }}

.auth-form-panel {{ padding: 2.4rem 2.4rem 2rem; }}
.auth-form-heading {{ font-size: 1.3rem; font-weight: 700; color: #edfaff; margin-bottom: .2rem; }}
.auth-form-subheading {{ color: #8ea9c3; font-size: .87rem; margin-bottom: 1.2rem; }}

[data-testid="stTabs"] button[role="tab"] {{ color: #7f9bb8 !important; font-weight: 600; font-size: .95rem; padding-bottom: .6rem; }}
[data-testid="stTabs"] button[aria-selected="true"] {{ color: #eafcff !important; }}
[data-testid="stTabs"] [data-baseweb="tab-highlight"] {{
    background: linear-gradient(90deg, #53c8ff, #9beaff) !important; height: 3px !important; border-radius: 3px;
}}
[data-testid="stTabs"] [data-baseweb="tab-border"] {{ background: rgba(83,230,255,.1) !important; }}
...
.auth-form-panel div[data-testid="stFormSubmitButton"] button,
.auth-form-panel .stButton > button {{
    width: 100%; border-radius: 10px; font-weight: 600; padding: .6rem 0;
    background: rgba(83,230,255,.09) !important; border: 1px solid rgba(83,230,255,.28) !important; color: #eafcff !important;
    transition: all .15s ease;
}}
.auth-form-panel div[data-testid="stFormSubmitButton"] button:hover,
.auth-form-panel .stButton > button:hover {{
    background: linear-gradient(90deg, rgba(83,230,255,.28), rgba(155,234,255,.22)) !important;
    border-color: rgba(83,230,255,.6) !important; transform: translateY(-1px);
}}
.auth-link-button button {{
    background: transparent !important; border: none !important; color: #6fc4ea !important;
    font-weight: 500 !important; font-size: .82rem !important; padding: .2rem 0 !important;
    text-align: left !important; box-shadow: none !important; width: auto !important;
}}
.auth-link-button button:hover {{ color: #9beaff !important; text-decoration: underline; transform: none; }}

/* ============ Branded full-screen loader ============ */
.lex-loader-overlay {{
    position: fixed; inset: 0; z-index: 9999; background: rgba(4,8,16,.76); backdrop-filter: blur(6px);
    display: flex; align-items: center; justify-content: center; animation: lexFadeIn .18s ease-out;
}}
.lex-loader-box {{
    display: flex; flex-direction: column; align-items: center; gap: .9rem; padding: 2.1rem 2.8rem;
    background: rgba(12,19,31,.96); border: 1px solid rgba(83,230,255,.35); border-radius: 16px;
    box-shadow: 0 0 30px rgba(83,230,255,.22), 0 24px 50px rgba(0,0,0,.5);
}}
.lex-loader-ring {{
    width: 44px; height: 44px; border-radius: 50%; border: 3px solid rgba(83,230,255,.15);
    border-top-color: #53e6ff; animation: lexSpin .75s linear infinite;
}}
.lex-loader-text {{ color: #eafcff; font-weight: 700; font-size: .96rem; letter-spacing: .2px; }}
.lex-loader-sub {{ color: #8fa8c2; font-size: .78rem; margin-top: -.4rem; }}
@keyframes lexSpin {{ to {{ transform: rotate(360deg); }} }}
...
/* ============ Mobile ============ */
@media (max-width: 860px) {{
    .auth-wrap {{ margin: .8rem auto 0; padding: 0 .4rem; }}
    .auth-shell {{ grid-template-columns: 1fr; border-radius: 18px; }}
    .auth-brand-panel {{ border-right: none; border-bottom: 1px solid rgba(83,230,255,.1); padding: 1.8rem 1.4rem; align-items: center; text-align: center; }}
    .auth-feature-list {{ align-items: center; }}
    .auth-feature {{ text-align: left; }}
    .auth-form-panel {{ padding: 1.8rem 1.4rem; }}
    .chat-text {{ font-size: 1.08rem; }}
    .lexassist-brand-title.main {{ font-size: 1.5rem; }}
}}
</style>
"""
...
def render_copy_button(content: str, message_id: int | None) -> None:
    ...
    components.html(
        f"""
        <style>
            body {{ margin: 0; background: transparent; color-scheme: dark; }}
            button {{
                cursor: pointer; border: 0; border-radius: 6px; background: transparent;
                color: #8fa8c2; font-size: 15px; line-height: 1; padding: 2px 4px;
                transition: all .15s ease;
...
def show_auth_screen() -> None:
    ...
    with left:
        logo_html = (
            f'<img class="auth-logo-img" src="data:image/png;base64,{LOGO_AUTH_B64}" alt="LexAssist">'
            if LOGO_AUTH_B64
            else '<div class="auth-brand-icon">⚖️</div><div class="auth-brand-name">LexAssist</div>'
        )
        st.markdown(
            f"""
            <div class="auth-brand-panel">
                ...
                if password != confirm_password:
                    ...
                    if error_message:
                        ...
                        if result.get("verification_code"):
                            ...
                            st.info("Check your email for a verification code.")
...
st.markdown('<div class="lexassist-brand-title main">LexAssist</div>', unsafe_allow_html=True)
st.caption("Ask about a legal topic in plain language. Include your country or state when it matters.")
